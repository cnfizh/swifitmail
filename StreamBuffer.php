    ...
    protected function _commit($bytes)
    {
        if (isset($this->_in)) {
            ...
            while ($totalBytesWritten < $bytesToWrite) {
                stream_set_blocking($this->_in, 1);   // _in有值时,设为阻塞模式
                stream_set_timeout($this->_in, 3);    // 设置超时时间
                $status = stream_get_meta_data($this->_in);  // 获取状态
                if ($status['timed_out']) {                 // 超时则进入下一次循环
                    continue;
                }
